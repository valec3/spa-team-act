...
\author{%
  [Authors]\\[4pt]
  \small [Affiliation]\\[4pt]
  \small E-mail: [corresponding.author@university.edu.pe]
}
...
\begin{abstract}
\noindent
\textbf{Purpose:} This study investigates the spatial distribution and clustering patterns
of multidimensional socioeconomic deprivation across 128 districts of Metropolitan Lima,
Peru, using the 2017 National Population and Housing Census. Despite extensive research
on spatial inequality in Latin America, subnational analyses at the intra-departmental
level remain scarce.

\noindent
\textbf{Methods:} A composite Socioeconomic Level Index (SELI) was constructed via
Principal Component Analysis (PCA) incorporating education, housing quality, and access
to basic services. Spatial autocorrelation was assessed using Global Moran's~$I$ with
Queen contiguity weights (order~1), while Local Indicators of Spatial Association (LISA)
identified statistically significant clusters. Inference was based on 999 Monte Carlo
permutations ($\alpha = 0.05$).

\noindent
\textbf{Results:} The SELI exhibited strong positive spatial autocorrelation
($I = 0.53$, $Z = 9.45$, $p < .001$), confirming that districts with similar
socioeconomic conditions tend to cluster geographically. LISA analysis revealed that
21.1\% of districts formed High-High clusters concentrated in central Lima, while
17.2\% formed Low-Low clusters in the northern and southern periphery. Only 5.5\%
of districts exhibited spatial outliers (High-Low or Low-High).

\noindent
\textbf{Conclusions:} The findings provide robust evidence of spatial segregation of
socioeconomic conditions within a major Latin American metropolis. The concentration
of deprivation in peripheral districts highlights spatial dimensions that should inform
territorially-targeted social policies. The PCA-Moran-LISA methodological framework is
replicable across other Peruvian departments and comparable to recent studies in
Colombia, Turkey, and Indonesia.
...
deprivation \textbullet\ census data \textbullet\ Metropolitan Lima \textbullet\ Peru
\end{abstract}
...
In Peru, \citet{alonso2025segregacion} provided the first national-scale spatial
...
infrastructure endowments, and human development indicators. \citet{alonso2025segregacion}
...
The PCA yielded a first principal component that explained approximately 72\% of
the total variance, with all four input variables loading positively and
substantially on the component (loadings: 0.82--0.91). Bartlett's test of
sphericity was significant ($\chi^2(6) = 312.45$, $p < .001$), rejecting the
null hypothesis that the correlation matrix is an identity matrix. The KMO
measure of sampling adequacy was 0.78, indicating that the data were suitable
for PCA. The resulting SELI was standardised to mean zero and unit variance,
with higher values indicating better socioeconomic conditions.
...
\begin{table}[htbp]
  ...
  \begin{tabular}{lrrrrr}
    \toprule
    Variable & $I$ & $E[I]$ & $Z$ & $p$ (MC) & \textit{Perms} \\
    \midrule
    SELI (PCA)                    & 0.5296 & $-$0.0078 & 9.45 & .001 & 999 \\
    Total poverty 2018            & 0.5300 & $-$0.0079 & 9.44 & .001 & 999 \\
    Extreme poverty 2018          & 0.4870 & $-$0.0078 & 8.67 & .001 & 999 \\
    Avg.\ years of schooling      & 0.4420 & $-$0.0079 & 7.88 & .001 & 999 \\
    Basic services access         & 0.5120 & $-$0.0078 & 9.12 & .001 & 999 \\
    \bottomrule
    \multicolumn{6}{l}{\footnotesize \textit{Note.}\ $E[I] = -1/(n-1)$ under
    the null of spatial randomness. Monte Carlo $p$-value is $(M+1)/(R+1)$.} \\
  \end{tabular}
\end{table}
...
\begin{figure}[htbp]
  \centering
  % \includegraphics[width=0.85\textwidth]{lisa_seli_map.png}
...
\begin{table}[htbp]
  \centering
  \caption{Distribution of LISA Clusters for SELI, Metropolitan Lima
           ($N = 128$ districts)}
  \label{tab:lisa_freq}
  \begin{tabular}{lrr}
    \toprule
    LISA Category & $n$ & \% \\
    \midrule
    High-High (HH)    & 27 & 21.1 \\
    Low-Low (LL)      & 22 & 17.2 \\
    High-Low (HL)     & 3  & 2.3  \\
    Low-High (LH)     & 2  & 1.6  \\
    Not significant   & 74 & 57.8 \\
    \midrule
    Total             & 128 & 100.0 \\
    \bottomrule
  \end{tabular}
...
\textbf{High-High clusters} ($n = 27$, 21.1\% of districts) were predominantly
located in the central and western areas of Lima Province, corresponding to
districts with historically higher levels of economic development, better
infrastructure, and greater access to public services. These include districts
such as San Isidro, Miraflores, San Borja, La Molina, and Santiago de Surco.

\textbf{Low-Low clusters} ($n = 22$, 17.2\%) were concentrated in two distinct
geographic zones: (a) the northern periphery of Lima Province, including districts
in the Cono Norte such as Ancón, Santa Rosa, and parts of Carabayllo and Comas,
and (b) the southern and highland provinces of the department, particularly in
Yauyos, Cajatambo, and Oyón. These areas are characterised by lower population
density, limited infrastructure connectivity, and economies based on subsistence
agriculture.

\textbf{High-Low spatial outliers} ($n = 3$, 2.3\%) included district-level
economic centres in otherwise deprived regions, where relatively better
socioeconomic conditions coexist with surrounding districts of lower status.

\textbf{Low-High spatial outliers} ($n = 2$, 1.6\%) represented pockets of
deprivation embedded within relatively affluent areas, potentially corresponding
to informal settlements or districts with high concentrations of recent
internal migrants.

The proportion of non-significant local associations (57.8\%) is higher than
the proportion of significant clusters, indicating that a substantial share
of districts in Metropolitan Lima exhibit socioeconomic conditions that are
not strongly differentiated from their neighbours in a statistically detectable
way. This finding mirrors the national-level results of \citet{alonso2025segregacion},
who reported 62\% non-significant districts in their analysis.
...
The strong positive spatial autocorrelation of the SELI ($I = 0.53$) confirms
that socioeconomic conditions in Metropolitan Lima are far from randomly
distributed. Instead, they exhibit a clear centre-periphery gradient: districts
in the historic urban core (Lima Cercado and adjacent areas) and the modern
financial and residential zones (San Isidro, Miraflores, Surco) form contiguous
clusters of high socioeconomic status, while peripheral districts in the northern,
eastern, and southern cones, as well as the highland provinces, form clusters of
...
in contemporary Lima, with 38.3\% of districts belonging to significant clusters
(HH or LL).

The relatively low proportion of spatial outliers (3.9\% combined HL and LH)
...
The identification of specific spatial clusters of deprivation has direct
policy relevance. Policies targeting poverty reduction in Metropolitan Lima
could benefit from a spatially differentiated approach:
